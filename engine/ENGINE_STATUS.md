...
## 播放器打磨:回看 / 存档 / 设置(2026-08-13)
- **回看可点击回跳**:每句台词记入 backlog 时保存 `(file, effectIndex)`;点任意一行回到该句并重放视觉(
  跨文件回跳自动按需重载目标剧本,缓存内零额外网络;选择等待态下禁用;重放期间不重复写历史)。
  `replay-check.mjs` 验证 111 条台词回跳一致性(全绿)。界面提示「点击任意一条可回到该句」。
- **读档恢复 BGM**:存档改为持久化 BGM 名(`currentBgmName`)而非易失的 blob URL;读档后按名重新懒加载。
- **读档精确恢复(续)**:存档额外记录场景内位置(`currentDlgIdx → effectIndex`);读档用同一 `replayToDialogue`
  机制恢复到**存档时的同一句**,而非从头重放该场景(旧存档无 effectIndex 时仍回退到从头播放)。
  槽位显示 `· 步N` 与 `♪`(BGM)标记。`replay-check.mjs` 增补「存档→读档精确恢复」断言(全绿)。
- **设置增强**:自动推进间隔随台词长度自适应(短句快、长句慢);新增「重置设置为默认」按钮(恢复并落盘)。
- **选择入回看**:选择点作出选择后,该行为写入对话历史(`who=选择`),回看记录更完整。
- **读档 UX**:读档模式空槽置灰(`:disabled`)并提示「空(无存档)」,不再点了没反应。
- **内存小修复**:背景/立绘 blob URL 每类仅保留最近 3 个并 revoke 旧值,长流程不无限泄漏。
- **修复控制栏误推进**:控制栏(`自动/跳过/回看/存档/读档/标题`)嵌在 `#stage` 内,原点击会冒泡到舞台
  → `manualAdvance`,导致点「自动」被立刻 `stopAuto` 关掉、点其它按钮还多推一步;🔊 重播也会误推进。
  现舞台点击/滚轮忽略 `.controls`,🔊 加 `stopPropagation`。
- **快捷操作**:`Ctrl+S` 快存槽 1,`Ctrl+L` 快读槽 1。
- 以上均为纯前端改动(`web/main.js` / `index.html` / `style.css`),不动 WASM 引擎与后端。
